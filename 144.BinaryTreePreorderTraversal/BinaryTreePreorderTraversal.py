...
class Solution:
    def preorderTraversal(self, root: Optional[TreeNode]) -> List[int]:
        prelist = []
        self.preorder_helper(root, prelist)
        return prelist

    def preorder_helper(self, node: TreeNode, prelist: list):
        if (node != None):
            prelist.append(node.val)
            self.preorder_helper(node.left, prelist)
            self.preorder_helper(node.right, prelist)
...
if __name__ == '__main__':
    ...
    So = Solution()
    ans = So.preorderTraversal(root.root_node)
    print(ans)
